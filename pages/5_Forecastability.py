...
if adfuller_result[1] > 0.05:
    st.error('Given time series data is non-stationary as p-value is more than 0.05, hence, data needs to be differenced.')
    adf_df = df_ad.copy()
    adf_df['diff1'] = adf_df[val_col].diff(periods=1)
    adf_df.dropna(inplace=True)
    adfuller_result = adfuller(adf_df['diff1'].values)
    st.info('Augmented Dickey_fuller test p-value after first order differencing: %f' % adfuller_result[1])
    if adfuller_result[1] > 0.05:
        st.error('Given time series data will require more levels of differencing.')
else:
